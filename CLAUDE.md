...
이 문서는 프로젝트의 **단일 진실 원천(Single Source of Truth)**이자, Claude가 작업 시
반드시 따라야 하는 **작업 틀(operating framework)**입니다. 모든 구현은 이 문서의 규칙과
컨벤션 안에서만 이루어집니다.
...
Agent Workflow**로 자동화한다.

---

## 2. 기술 스택 + 선택 근거

이 프로젝트는 **학습용**이다. 각 선택의 **근거를 명시**해 "왜 이 기술을 쓰는가",
"대안 대비 어떤 트레이드오프가 있는가"를 이해하고 넘어가는 것을 목표로 한다.
...
- **MySQL vs PostgreSQL+pgvector**: MySQL을 메인으로 택했기에 단일 DB 통합(pgvector)은 포기.
  대신 **관계형은 MySQL, 벡터는 Qdrant로 역할 분리**하여 책임을 명확히 했다.
- **전용 벡터 DB(Qdrant) 선택**: 이 규모에서 전용 벡터 DB가 정당화되는지 검토 → 메타 필터링과
  생태계 성숙도, 로컬 시연성을 근거로 채택.
- **VectorStore 추상화 필수**: 코드는 `VectorStore` 인터페이스에만 의존. `QdrantVectorStore`는
  구현체. 규모/요구가 바뀌면 교체 가능하도록 설계한다. (이부분이 핵심)
- **임베딩 모델**: OpenAI `text-embedding-3-small` (**1536차원**) 확정. 데이터는 한국어 본문 +
  영어 기술용어(API명·에러코드·코드)가 섞인 형태로 가정 → 다국어 모델 채택. **Qdrant 컬렉션은
  1536차원으로 생성**한다. 모델 변경 시 차원이 바뀌면 재인덱싱 필요.
- **임베딩과 LLM은 별개**: 답변 생성 LLM은 임베딩과 독립적으로 선택한다(미정). 특히 Claude는
  임베딩을 제공하지 않으므로 LLM을 무엇으로 정하든 임베딩은 OpenAI를 사용한다.

---

## 3. 아키텍처 원칙

- **Agent는 Tool을 통해서만 행동한다.** Agent가 DB/외부를 직접 조회하지 않는다.
  모든 행동은 명시적 Tool 단위로 분리한다.
- **RAG는 근거 기반(grounded)이다.** 출처 없는 답변을 생성하지 않는다. score·confidence·
  needHumanReview를 응답에 포함한다.
- **상태는 명시적으로 관리한다.** Agent 워크플로우 상태(`IssueAnalysisState`)를 타입으로 정의한다.
...
새 파일은 이 구조 안에만 생성한다. 구조 변경이 필요하면 먼저 제안·승인받는다.
...
- **작업 단위 커밋**: 한 번에 많은 양을 몰아서 커밋하지 않고, **의미 있는 작업 단위마다**
  나눠서 커밋한다. 하나의 커밋은 하나의 목적(기능/수정/리팩터링)만 담는다.
  변경 이력이 곧 학습 기록이 되도록 작게·자주 커밋하는 것을 원칙으로 한다.
- **커밋 시점**: 작업 단위가 끝나면 Claude가 커밋 메시지를 제안하고, 사용자 승인을 받아
  커밋한다. 승인 없는 자동 커밋·푸시는 하지 않는다.
...
> Claude는 아래 틀 안에서만 동작한다.

- **계획 승인(중간 강도)**: 다음 경우 구현 전 계획을 제시하고 승인받는다 —
  ① 새 모듈/기능 추가 ② 큰 구조 변경 ③ 새 의존성 추가.
  단순 수정·버그픽스·소규모 변경은 바로 진행한다.
- **단일 모듈 집중**: 한 번에 한 모듈/관심사만 작업한다.
- **의존성 통제**: 새 라이브러리 추가는 사전 승인 필수 (YAGNI 준수).
- **검증 강제**: `typecheck` · `lint` · `test`가 통과하기 전에는 "완료"라고 선언하지 않는다.
- **응답 형식**: 작업 보고는 `[목표] → [변경 파일] → [검증 방법]` 구조를 따른다.
- **진행 추적**: 다단계 작업은 할 일 목록으로 진행상황을 표시한다.
- **소통 언어**: 한국어. 코드 주석/커밋은 위 컨벤션을 따른다.
- **구조 준수**: 4번 디렉터리 구조 밖에 파일을 만들지 않는다.
